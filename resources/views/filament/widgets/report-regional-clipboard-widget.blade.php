<x-filament::widget>
    ...
    showSuccess() {
        ...
        new FilamentNotification()
            .title('¡Reporte Regional copiado!')
            .success()
            .send();
...
class="flex flex-col md:flex-row items-center justify-between gap-4"
        >
            <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                    <x-heroicon-o-clipboard-document-list class="w-6 h-6 text-warning-600 dark:text-warning-400"/>
                    <h2 class="text-lg font-bold text-gray-800 dark:text-white">Reporte Regional (WhatsApp)</h2>
                </div>
            ...
            <button 
                ...
                :class="copied ? 'bg-green-600 hover:bg-green-700' : 'bg-warning-600 hover:bg-warning-700 disabled:opacity-50 disabled:cursor-not-allowed'"
                class="w-full md:w-auto inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-warning-500 transition-all duration-200"
            >
        ...
        <div x-data="{ open: false }" class="mt-4 border-t border-gray-200 dark:border-gray-700 pt-4">
            <button @click="open = !open" class="text-xs text-gray-500 hover:text-warning-600 flex items-center">
